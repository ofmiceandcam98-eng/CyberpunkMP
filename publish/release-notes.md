...
- **New: the Checkup.** One automated checklist over everything between double-click and playing — launcher version, game found and writable, mod and its exact version, all six frameworks, Discord and Nexus sign-ins, the Mod Manager Download handler, **which server JACK IN will actually use**, Tailscale, whether the server answers, and whether every installed mod's files are still on disk. It runs by itself at startup and lives in Settings; every failing row says what to do about it. Born from a real evening lost to a launcher that was quietly pointed at the wrong server.
