...
# ---------------------------------------------------------------------------
# Preconditions a build cannot supply
# ---------------------------------------------------------------------------

# The Rpc bindings are the one payload part nothing here produces: RpcGenerator makes them
# by LAUNCHING THE GAME with --rpc, so a checkout that never did that has none. Checked here,
# before minutes of build and Verify, because on 2026-09-12 it was found by a raw Copy-Item
# error at the very end instead.
if (-not $WhatIf -and -not (Test-Path (Join-Path $Repo "distrib\launcher\mod\Rpc"))) {
    Die ("distrib\launcher\mod\Rpc is missing - it is generated by running the game with --rpc, not by the build.`n" +
         "        fix: carry it from a previous build (another checkout's distrib\launcher\mod\Rpc, or the installed`n" +
         "        mod's Rpc folder), ONLY after 'git diff <that build's commit>..HEAD -- code/protocol/' is empty`n" +
         "        (docs/MAP.md, 'distrib/launcher/mod/Rpc does not exist on a fresh checkout').")
}
...
if (-not $WhatIf) {
    ...
    #
    # A MIRROR, not an overlay: the folder is emptied first. Copying over the top only ever
    # ADDS, so a script deleted or renamed in source lived on in distrib and shipped - test.33
    # carried CharacterProfile.reds and KiroshiScanner.reds, which have never been in git.
    # Every .reds compiles as one unit, so a stale one is not harmless: it is how a duplicate
    # class takes the whole mod down.
    ...
    Ok "redscript mirrored from source"
    ...
    # ARCHIVES TOO - and this one shipped a broken build. The .archive files are TRACKED in
    # git (code\assets\Archives\packed\archive\pc\mod) and only the Archives target's
    # on_install ever put them in distrib. So a checkout that had been built but never
    # installed shipped a payload with no archives at all: test.35 (2026-09-12) went out with
    # no CyberpunkMP.archive - the mods\cyberpunkmp\*.ent the muppet records spawn from - and
    # no clean-start archive, and every check still passed. test.33 was missing the Selector
    # archive the same way. Mirrored, and a payload with none is refused outright.
    $archiveSrc = Join-Path $Repo "code\assets\Archives\packed\archive\pc\mod"
    $archiveDst = Join-Path $modDir "assets\Archives"
    $archives = @(Get-ChildItem $archiveSrc -Filter *.archive -File -ErrorAction SilentlyContinue)
    if ($archives.Count -eq 0) { Die "no .archive files in $archiveSrc - a payload without them spawns no remote players" }
    if (Test-Path $archiveDst) { Remove-Item $archiveDst -Recurse -Force }
    New-Item -ItemType Directory -Force -Path $archiveDst | Out-Null
    $archives | ForEach-Object { Copy-Item $_.FullName $archiveDst -Force }
    Ok "archives mirrored from source ($($archives.Count): $(($archives | ForEach-Object Name) -join ', '))"
