...
struct Config : IConfig
{
    ...
    // Both sides of the post-migration merge added a field here. CoalesceMovement came from
    // the Phase 5 branch, ServerListEndpoint from the server-list fix during the swap - and a
    // field missing from this macro is not a compile error, it is a setting that silently
    // stops being read from server.json. Keeping both is the whole resolution.
    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(Config, Name, Description, IconUrl, MaxPlayer, Tags, TickRate, UpdateRate, CoalesceMovement, Public, Port, Password, ApiKey, ServerListEndpoint, Flecs, Discord)
};