...
Each increment is independently live-testable by the standing two-player protocol: both clients on the SAME stack, grep `[Identity]`/`[Appearance]` lines in `/mnt/vol/projects/CyberpunkMP/logs/clients/<player>/` plus the server log, with increment-specific pass criteria as listed per workstream above.
